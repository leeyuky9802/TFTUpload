...
var baifenbi        = document.getElementById('percent-label');
...
function startUpload() {
  ...
  var endpoint = "http://yangxiao.site:27272/files";
  var chunkSize = parseInt(10000000, 10);
  ...
  var options = {
    ...
    onProgress: function (bytesUploaded, bytesTotal) {
      ...
      baifenbi.innerHTML = percentage.toString() + '%';
      console.log(bytesUploaded, bytesTotal, percentage + "%");
    },
    onSuccess: function () {
      var anchor = document.createElement("a");
      anchor.textContent = "已成功 " + upload.file.name + " ( 共计 " + upload.file.size + " bytes)";
      //anchor.href = upload.url;
      
      var ws = new WebSocket("ws://yangxiao.site:8887");
      var hahaha = upload.url.toString()+ "  " + upload.file.name;
      ws.onopen = function() {
        ws.send(hahaha);
      }
...
function askToResumeUpload(previousUploads, upload) {
  ...
  // console.log(previousUploads.length);
  ...
  var index = previousUploads.length;
  index = index -1;
